...
def Newton(x0,f,g,hessiano): 
    ...
    while norm>=tol and i<max_iter:
        dk = eliminacion_gaussiana_piv(hessiano(xk), -g(xk))
        xk=xk +dk
        gk = g(xk)
        norm=la.norm(gk)
        i=i+1
...
#%% Probamos algoritmo de descenso, newton y newton modificado
...
for j in range(n):
    ...
    #ROSEMBROCK
    t_steepest0 = time()
    i_steepest += steepest_des(x_new,f,gradRosn,metodo=1)
...
for j in range(n):
    ...
    #ROSEMBROCK
    t_steepest0_w = time()
    i_steepest_w+=steepest_des(x_new_w,wood,gwood,metodo=1)
